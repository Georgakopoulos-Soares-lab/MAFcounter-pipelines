...
# Orchestrator script to run multiple Slurm jobs (gerbil_benchmark_single.sh)
...
SLURM_SCRIPT="gerbil_benchmark_single.sh"
RESULTS_DIR="./results"
WORKING_DIR="/scratch/kap6605/gerbil_bench"
...
# Choose parameters set: "default" or "FINETUNED"
PARAMS="FINETUNED"

# Determine suffix based on PARAMS for result filenames
...
for FILE in "${FILES[@]}"; do
  for K in "${K_VALUES[@]}"; do
    ...
    # Clear working directory before each job
    rm -rf "${WORKING_DIR}/"*
    ...
    # Parse out State, MaxRSS, Elapsed, MaxDiskWrite from the '|' separated line
