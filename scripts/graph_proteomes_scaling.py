...
def parse_maxrss(maxrss_str):
    ...
    try:
        if maxrss_str.endswith('K'):
            ...
            # Assume the number is in KB if no suffix
...
def plot_scaling(df, filename, metric, ylabel, output_file):
    ...
    # Place legend: memory = best, time = upper right
    loc = "upper right" if metric == 'Elapsed_sec' else "best"
    plt.legend(loc=loc)
...
def main():
    ...
    csv_path = "/storage/group/izg5139/default/multiple_alignment/maf_counter_migration/pipelines/maf_counter_benchmark/results_proteomes_test/job_resources_test.csv"
    ...
    for fname in filenames:
        ...
        # Memory plot
        output_mem = f"{base}_memory_scaling.png"
        plot_scaling(df, fname, 'Memory_MB', "Memory (MB)", output_mem)
        # Time plot
        output_time = f"{base}_time_scaling.png"
