...
echo "Running maf_counter_proteomes_count test job with:"
...
run_maf_counter() {
    ...
    echo "Starting maf_counter_proteomes_count for ${file_name} with k=${kmer_size} using ${total_cores} cores"
    ...
    maf_counter_proteomes_count --binary_file_output "${output_file}" \
                       --k "${kmer_size}" \
                       --threads "${total_cores}" \
    ...
    echo "maf_counter_proteomes_count for ${file_name} with ${total_cores} cores took ${elapsed_time} seconds."
